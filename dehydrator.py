...
9. 如果原文里的 user / 用户 / 用户消息指的是这段关系里的当前用户，请在 content 中写作 {user_display_name}；如果 assistant / AI / 模型 / 助手消息指的是这段关系里的当前回应者，请写作 {ai_name}。不要写成泛称 user、AI、assistant 或模型。，不要在 {user_display_name} 和 {ai_name} 外面加 [[]] 双括号，直接写名字即可
...
13. content 分段时使用：### moment 写真实发生的事件/经历；### original 写当时原话或证据文本；### reflection 写 {ai_name} 的理解、确认、偏爱原因或以后如何回应；### followup 写后续承诺、待办、选择或状态变化；### affect_anchor 只允许一行和弦、bpm、力度等温度线，例如 `> Dm9 -> G13sus4 -> Cmaj9 · 60bpm · mp`，不要写普通文字、场景、含义解释、事件正文、原话或反思。不是每个条目都需要全部 section，没有的不写。
...
8. 如果输出中包含分段，使用 ### moment / ### original / ### reflection / ### followup / ### affect_anchor；affect_anchor 只允许一行和弦、bpm、力度等温度线，不要放普通文字、场景、含义解释、事件描述、原话或反思。
