...
```mermaid
flowchart LR
    C["聊天客户端"] --> G["Ombre Gateway :8010"]
    G --> U["上游聊天模型"]
    G --> B["Ombre Brain :8000"]
    M["MCP 客户端"] --> B
    D["Dashboard"] --> B
...
| 组件 | 作用 | 主要入口 |
| --- | --- | --- |
| Ombre Brain | MCP 工具、记忆读写、召回、画像维护、Dashboard | `server.py`，默认 `:8000` |
| Ombre Gateway | 转发聊天请求，并在请求前注入经过门控的上下文 | `gateway.py`，默认 `:8010` |
