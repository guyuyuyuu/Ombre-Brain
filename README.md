...
- Darkroom 用来放未想透、不该给用户看、不该进普通记忆的内在反思；外部工具清单只暴露 `darkroom_enter` 和受锁门时间限制的只读 `darkroom_view`。
...
| Darkroom | 保存未想透、不该给用户看、不该进普通记忆的内在反思；写入可带 `lock_for`，外部工具清单只开放 `darkroom_enter` 和只读 `darkroom_view` | `darkroom.py`、`server.py`、`dashboard.html` |
...
| `darkroom_enter` | 写入私密暗房，可传 `lock_for="6h"` / `"3d"`，只返回门口状态，不回显正文。 |
| `darkroom_view` | 只读查看已解锁的暗房内容；未到锁门时间只返回 `unlock_at`，不返回正文。 |
